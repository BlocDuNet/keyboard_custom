...
class KeyboardApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Keyboard Customizer PRO - v4.2 (Fixed Syntax)")
    ...
    def setup_ui(self):
        ...
        self.kb_tree.heading("ID", text="ID")
        self.kb_tree.heading("Nom", text="Nom")
        self.kb_tree.column("ID", width=100)
        ...
        btn_box = ttk.Frame(self.left_frame)
        ...
        self.cfg_frame = ttk.LabelFrame(self.right_frame, text="Actions du Clavier", padding="10")
        self.cfg_frame.pack(fill=tk.X, pady=5)
        self.sel_label = ttk.Label(self.cfg_frame, text="Sélectionnez un clavier", font=("Arial", 9, "bold"))
        self.sel_label.pack(pady=5)

        entry_frame = ttk.Frame(self.cfg_frame)
        entry_frame.pack(fill=tk.X, pady=5)
        ttk.Label(entry_frame, text="Touche :").grid(row=0, column=0)
        self.ent_from = ttk.Entry(entry_frame, width=12)
        ...
        self.ent_to = ttk.Entry(entry_frame, width=35)
        self.ent_to.grid(row=2, column=1, columnspan=2, sticky="ew", padx=5)
        ...
        ttk.Button(self.cfg_frame, text="ENREGISTRER CETTE ACTION", command=self.add_mapping).pack(fill=tk.X, pady=10)

        self.mapping_list = tk.Listbox(self.cfg_frame, height=6, font=("Consolas", 10))
    ...
    def browse_script(self):
        ...
        if f:
            ...
            self.ent_to.insert(0, f)
            self.action_type.set("Script")
        ...
        while not self.ui_queue.empty():
            ...
            if task == "log":
                ...
                if field == "from":
                    ...
                    self.ent_from.insert(0, val)
                else:
                    self.ent_to.delete(0, tk.END)
                    self.ent_to.insert(0, val)
        self.root.after(100, self.process_ui_tasks)

    def refresh_kb_list(self):
        for i in self.kb_tree.get_children():
            self.kb_tree.delete(i)
    ...
    def on_kb_select(self, e):
        ...
        if info:
            self.sel_label.config(text=f"Configuration : {info['name']} ({h})")
    ...
    def refresh_mapping_list(self, h):
        ...
        for f, data in mappings.items():
            self.mapping_list.insert(tk.END, f"{f} -> [{data.get('type')}] {data.get('value')}")

    def start_capture(self, target):
        self.is_capturing = True
        self.capture_target = target
        self.log(f"Capture en cours pour {target}...", "match")
    ...
    def start_identification(self):
        ...
        self.log(">>> MODE IDENTIFICATION ACTIF. Pressez une touche.", "match")
    ...
    def rename_keyboard(self):
        ...
        if name:
            self.config["keyboards"][h]["name"] = name
            self.save_config()
            self.refresh_kb_list()
    ...
    def raw_input_loop(self):
                ...
                if size.value > 0:
                    ...
                    if raw.header.dwType == RIM_TYPEKEYBOARD:
                        ...
                        with self.lock:
                            ...
                            if self.is_capturing and is_down:
                                self.is_capturing = False
                                self.ui_queue.put(("set_entry", (self.capture_target, hex(vk))))
                            if self.is_identifying and is_down:
                                ...
                                self.log(f"ID SUCCESS: {h_str}", "match")
        ...
        wc.lpszClassName = f"KBC_RawInput_{int(time.time())}"
            ...
            if nCode >= 0:
                ...
                with self.lock:
                    ...
                    for ev in reversed(self.raw_events_buffer):
                        if ev["vk"] == kb.vkCode and abs(time.time() - ev["time"]) < 0.2:
                            match = ev
                            break
                if match:
                    h_str = str(match["h"])
    ...
    def press_key(self, vk, is_down=True):
        ...
        inp.type = 1
        inp.u.ki.wVk = vk
        inp.u.ki.dwFlags = 0 if is_down else 2
